...
import { scanDirectories, getCommonStartPaths } from "../services/directory-scanner.ts"
...
export function FuzzyFinder({ onSelect, onCancel }: FuzzyFinderProps) {
  ...
  useEffect(() => {
    async function loadDirectories() {
      ...
      const allDirs: string[] = [...startPaths]
  ...
  useKeyboard((key) => {
    ...
    } else if (key.sequence && key.sequence.length === 1 && !key.ctrl && !key.alt) {
  ...
  return (
    <box flexDirection="column" padding={1}>
      <box border title="Select Directory" padding={1} flexDirection="column" gap={1}>
        {isLoading ? (
          <text>
          ...
          <>
                ...
                {filteredDirs.length === 0 ? (
                  ...
                  filteredDirs.slice(0, maxVisible).map((dir, index) => (
                    <text key={dir}>
                      ...
                      <span fg={index === selectedIndex ? "white" : "gray"}>{dir}</span>
                    </text>
                  ))
                )}
              </box>
            ...
            <text>
              <span fg="gray">
                {filteredDirs.length} results | [↑↓] Navigate [Enter] Select [Esc] Cancel
              </span>
            </text>
          </>
        )}
