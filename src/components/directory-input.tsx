...
export function DirectoryInput({
  ...
  useKeyboard((key) => {
    ...
    } else if (focused === "days" && (key.name === "left" || key.name === "right")) {
    ...
    <box flexDirection="column" padding={1}>
      <box border title="Daily Summary" padding={1} flexDirection="column" gap={1}>
        ...
        <box flexDirection="column" gap={1}>
          ...
          <box border backgroundColor={focused === "days" ? "#1a1a2e" : undefined} padding={1}>
            <text>
              <span fg={focused === "days" ? "cyan" : "gray"}>◀</span>
              {"  "}
              <span fg={focused === "days" ? "white" : "gray"}>{daysToInclude} {daysToInclude === 1 ? "day" : "days"}</span>
              {"  "}
              <span fg={focused === "days" ? "cyan" : "gray"}>▶</span>
            </text>
          </box>
        ...
        <text>
          <span fg="gray">[Tab] Switch fields  [←/→] Days  [Ctrl+F] Browse  [Enter] Start</span>
        </text>
      </box>
    </box>
  )
